...
<!---增加打包资源记录开始--->
<div id="popDiv2" class="mydiv package" style="display:none;"> 
	<h4>打包及发布资源框</h4>
	<div class="groups">
		<label>文件名称：</label><input class="filename" type="text" value="请输入打包文件名称"/>
	</div>
	<div class="groups" style="position:relative;">
		...
		<select name="dws" id="dws" onChange="javascript:document.getElementById('dw').value=document.getElementById('dws').options[document.getElementById('dws').selectedIndex].value;">
			...
			<option value='车/次'>车/次</option>
			<option value='床/日'>床/日</option>
			<option value='次'>次</option>
			<option value='次/侧'>次/侧</option>
			<option value='次/个'>次/个</option>
			<option value='次/双'>次/双</option>
		</select>
		</span>
		<span class="select_con">
			<input type="text" name="dw" id="dw" value="">
		</span>
		<span class="publish"><button type="submit">发布</button></span>
	</div>
...
<div class="mainbody">
    ...
    <div class="rig_lm03 Teach_res video_res asse_res res_making">
		<div id="tabCot_product" class="zhutitab">
			...
			<div id="tabCot_product_1" class="tabCot">  
				<div class="clearfix Admin_Info">
						...
						<div class="fr search">
							...
							<div class="am-dropdown fr" data-am-dropdown>
								...
								<ul class="am-dropdown-content">
								  ...
								  <li><a href="Teacher_Course_Statistics">2. 课程情况统计</a></li>
								  <li><a href="resources_package_Statistics">3. 课程资源打包及发布情况统计</a></li>
								</ul>
							</div>
						</div>
					</div>
					...
					<div class="userList">
					  <table id="special_table" class="tabindex" width="100%" border="0" cellpadding="0" cellspacing="0" id="tableresource">
						  <tr>
				...
				<?php $countNum = 1;?>
                         <?php if(is_array($showData)): $i = 0; $__LIST__ = $showData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$arr): $mod = ($i % 2 );++$i;?><tr  class="b_white">
							  ...
							  <td class="czcol">
							  	<p><a href="through_review?user[]=<?php echo ($arr[0]['userid']); ?>&course[]=<?php echo ($arr[0]['courseid']); ?>">审核通过</a></p>
							  	<!--<p><a href="#"  onClick="javascript:showDiv2()">打包并发布</a></p>-->
							  	<p><a href="#" onclick="publicres(<?php echo ($arr[0]['userid']); ?>,<?php echo ($arr[0]['courseid']); ?>)">发布</a></p>
...
<script type="text/javascript"  src="/eduresPro/Public/js/tab.js"></script>
...
<script type="text/javascript">   <!--课程资源打包-->
function showDiv2(){

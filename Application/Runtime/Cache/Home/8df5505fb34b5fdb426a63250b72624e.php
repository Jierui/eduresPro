		...
		<div id="tabCot_product" class="zhutitab">
			...
			<div id="tabCot_product_1" class="tabCot">  
				<div class="clearfix Admin_Info">
					<div class="buttons">
					   <div class="fl add">
						  <button type="button" onclick="evl_print()">打印</button>
						  <button type="button" onclick="del_evaluation()">删除</button>
					   </div>
						  
					  <div class="filtrate fl"	>
							<label>筛选：</label>
						  <select data-am-selected="{btnSize: 'sm'}" onchange="evl_filtrate(this)">
						...
						<div class="fr search">
							<input type="text" id="evltext">
							<button type="button" onclick="showTeacherEvalution(1,true)">搜索</button>
						</div>
					</div>
...
function score(a,id){
    ...
    document.getElementById("n").addEventListener("blur",function(e){
    	$.post('evaluation',{ID:id,data:document.getElementById("n").value},function(data){
    	},"json");
